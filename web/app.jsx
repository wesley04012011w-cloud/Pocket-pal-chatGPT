...
const MessageRow=memo(function MessageRow({message}){return <article className={'message-row '+(message.role==='user'?'user':'assistant')}><div className="message-role">{message.role==='user'?'You':'Assistant'}</div><div className="message-content"><Markdown text={message.content}/></div></article>});
...
function App(){
 ...
 const [drawer,setDrawer]=useState(false),[screen,setScreen]=useState('chat'),[settingsOpen,setSettingsOpen]=useState(false),[loading,setLoading]=useState(true);
 ...
 useEffect(()=>{
   const tokenListener=Llama.addListener('token',ev=>{
     ...
     const next={answer:p.answer,thinking:p.thinking,thinkingLive:p.active,stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};
     streamRef.current=next;setStream(next);scheduleCommit();
   });
   const doneListener=Llama.addListener('generationDone',()=>{const g=genRef.current,p=parseThinking(g.raw);const elapsed=Math.max(.001,(performance.now()-g.start)/1000);const next={answer:p.answer,thinking:p.thinking,thinkingLive:false,stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};streamRef.current=next;setStream(next);commitStream();setGenerating(false);genLiveRef.current=false});
 ...
 async function loadPath(path,name){try{const r=await Llama.loadModel({path,...cfg});if(r.ok){const n=name||path.split('/').pop();setModel(n);localStorage.setItem('pp_model',n);localStorage.setItem('pp_model_path',path);setScreen('chat')}}catch(e){console.error(e);alert('Model load error. Export Logs for diagnosis.')}}
 ...
 async function send(){if(genLiveRef.current)return;const text=input.trim();if(!text)return;if(!model){await pickAndLoad();return}const title=current.title==='New chat'?text.slice(0,42):current.title;const msgs=[...current.messages,{role:'user',content:text},{role:'assistant',content:'',thinking:''}];setChats(prev=>prev.map(c=>c.id===current.id?{...c,title,messages:msgs}:c));setInput('');const g={count:0,start:performance.now(),raw:''};genRef.current=g;const empty={answer:'',thinking:'',thinkingLive:false,stats:'0 tokens · 0.0 tok/s'};streamRef.current=empty;setStream(empty);setGenerating(true);genLiveRef.current=true;try{await Llama.generate({roles:msgs.slice(0,-1).map(x=>x.role),contents:msgs.slice(0,-1).map(x=>x.content),...cfg})}catch(e){console.error(e);setGenerating(false);genLiveRef.current=false;commitStream()}}
 function newChat(){const c=makeChat();setChats(p=>[c,...p]);setChatId(c.id);setScreen('chat');setDrawer(false);setInput('');setStream({answer:'',thinking:'',thinkingLive:false,stats:''})}
 ...
 return <div className="app">
  ...
  {screen==='models'?<ModelsPage downloaded={downloaded} model={model} downloading={downloading} onBack={()=>setScreen('chat')} onImport={pickAndLoad} onDownload={download} onLoad={loadPath} onUnload={unload}/>:<main className="shell">
   ...
   <header className="floating-top"><button className="float-btn" onClick={()=>setDrawer(true)} aria-label="Menu"><Icon name="menu"/></button><div className="float-model">{model||'No model loaded'}</div><div className="float-actions"><button className="float-btn" onClick={()=>document.querySelector('.input')?.focus()} aria-label="Focus input"><Icon name="edit"/></button></div></header>
   <section className="messages">{current.messages.length?<>{current.messages.map((m,i)=>{const last=i===current.messages.length-1;const shown=last&&generating&&m.role==='assistant'?{...m,content:stream.answer}:m;return <MessageRow key={i} message={shown}/>})}</>:<div className="empty"><div className="empty-mark"><Icon name="spark"/></div><h1>Local AI, on your phone.</h1><p>Load a GGUF model and chat privately. Thinking, streaming and conversation history stay on-device.</p><button className="load" onClick={pickAndLoad}>Select GGUF model</button></div>}</section>
   <div className="composer-wrap">
    {stream.thinkingLive&&<div className="thinking"><div className="thinking-head"><span className="brain"><Icon name="spark"/></span><span>Thinking...</span><span className="thinking-dots">•••</span></div><div className="thinking-body">{stream.thinking||' '}</div></div>}
