...
function renderMarkdown(src='',flash=0){
 ...
 for(let i=0;i<lines.length;i++){const line=lines[i];
  if(line.trim().startsWith('\x60\x60\x60')){if(inCode){out.push('<div class="md-code-wrap"><button class="md-code-copy" type="button" data-copy-code="'+escapeHtml(code.join('\\n'))+'">Copy</button><pre><code>'+escapeHtml(code.join('\n'))+'</code></pre></div>');code=[];inCode=false}else inCode=true;continue}
 ...
 let html=out.join('');
 if(Number(flash)>0)html=markLastText(html,flash);
 return html;
}
function markLastText(html,count=0){
 ...
 for(let i=parts.length-1;i>=0&&remaining>0;i--){
   ...
   for(let j=0;j<part.length;){
     if(part[j]==='&'){
       const m=part.slice(j).match(/^&(#[0-9]+|#x[0-9a-f]+|[a-z]+);/i);
       if(m){units.push(m[0]);j+=m[0].length;continue}
     }
     units.push(part[j]);j++;
   }
   if(!units.length)continue;
   const take=Math.min(remaining,units.length);
   const splitAt=units.slice(0,units.length-take).reduce((a,x)=>a+x.length,0);
   const base=part.slice(0,splitAt);
   const recent=units.slice(-take).join('');
   parts[i]=base+'<span class="generation-flash">'+recent+'</span>';
   remaining-=take;
 }
 return parts.join('');
}
...
const MessageRow=memo(function MessageRow({message,streaming=false,flash=0}){const [copied,setCopied]=useState(false);const copy=async()=>{try{await navigator.clipboard.writeText(String(message.content||''));setCopied(true);setTimeout(()=>setCopied(false),1200)}catch{try{const ta=document.createElement('textarea');ta.value=String(message.content||'');document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();setCopied(true);setTimeout(()=>setCopied(false),1200)}catch{}}};return <article className={'message-row '+(message.role==='user'?'user':'assistant')}><div className="message-role">{message.role==='user'?'You':'Assistant'}{message.role==='assistant'&&<button className="copy-message" onClick={copy} aria-label="Copy response"><Icon name="copy"/><span>{copied?'Copied':'Copy'}</span></button>}</div><div className="message-content"><Markdown text={String(message.content||'')} flash={streaming?flash:0}/></div></article>});
...
function App(){
 ...
 useEffect(()=>{localStorage.removeItem('pp_model');localStorage.removeItem('pp_model_path')},[]);
 ...
 useEffect(()=>{
   const tokenListener=Llama.addListener('token',ev=>{
     if(!ev?.text)return;
     const g=genRef.current,text=String(ev.text);
     g.count++;
     if(ev.thinking)g.thinking=(g.thinking||'')+text;else g.targetAnswer=(g.targetAnswer||'')+text;
     const elapsed=Math.max(.001,(performance.now()-g.start)/1000);
     const next={answer:g.targetAnswer||'',thinking:g.thinking||'',thinkingLive:!!ev.thinking,flash:ev.thinking?0:text.length,stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};
     streamRef.current=next;setStream(next);
   });
   const doneListener=Llama.addListener('generationDone',()=>{
     const g=genRef.current,elapsed=Math.max(.001,(performance.now()-g.start)/1000);
     const final={answer:g.targetAnswer||'',thinking:g.thinking||'',thinkingLive:false,flash:0,stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};
     streamRef.current=final;setStream(final);commitStream();setGenerating(false);genLiveRef.current=false;
   });
   return()=>{tokenListener.then(x=>x.remove());doneListener.then(x=>x.remove())}
 },[commitStream]);
 async function loadPath(path,name){
 const n=name||path.split('/').pop();
 if(loadingModel)return;
 setLoadingModel(n);
 try{
   const r=await Llama.loadModel({path,...engineCfg});
   if(r?.ok){setModel(n);setScreen('chat')}
   else alert('Model load failed. Export Logs for diagnosis.');
 ...
 setDownloadProgress({name:item.file,bytes:0,total:0,percent:0});
 try{await Llama.downloadModel({url:item.url,name:item.file});await refreshModels()}
 ...
 return <div className={'app theme-'+uiTheme+(performanceMode?' performance-mode':'')}>
  ...
  {screen==='models'?<ModelsPage downloaded={downloaded} model={model} downloading={downloading} downloadProgress={downloadProgress} loadingModel={loadingModel} onBack={()=>setScreen('chat')} onImport={pickAndLoad} onDownload={download} onLoad={loadPath} onUnload={unload}/>:<main className="shell">
   ...
   <section ref={messagesRef} className="messages">{current.messages.length?<>{current.messages.map((m,i)=>{const last=i===current.messages.length-1;const shown=last&&generating&&m.role==='assistant'?{...m,content:stream.answer}:m;return <MessageRow key={i} message={shown} streaming={last&&generating&&m.role==='assistant'} flash={last&&generating&&m.role==='assistant'?stream.flash:''} trail={last&&generating&&m.role==='assistant'?stream.trail:[]}/>})}</>:<div className="empty">{model?<><div className="empty-mark"><Icon name="spark"/></div><h1>What do you want to do?</h1><p>Your model is ready. Start with one of these ideas or write anything below.</p><div className="suggestions"><button onClick={()=>setInput('Explain something to me like I am a beginner.')}>Explain something simply</button><button onClick={()=>setInput('Help me plan my day.')}>Plan my day</button><button onClick={()=>setInput('Give me ideas for a project I can build.')}>Brainstorm a project</button><button onClick={()=>setInput('What can you help me with?')}>Show what you can do</button></div></>:<><div className="empty-mark"><Icon name="spark"/></div><h1>Local AI, on your phone.</h1><p>Load a GGUF model and chat privately. Thinking, streaming and conversation history stay on-device.</p><button className="load" onClick={()=>setScreen('models')}>Select GGUF model</button></>}</div>}</section>
...
function ModelsPage({downloaded,model,downloading,downloadProgress,loadingModel,onBack,onImport,onDownload,onLoad,onUnload}){
 return <div className="models-page"><div className="models-head"><button className="float-btn" onClick={onBack}><Icon name="back"/></button><div><b>Models</b><small>Download, import and manage GGUF files</small></div></div><div className="import-box"><button className="import-model" onClick={onImport} disabled={!!loadingModel||!!downloading}><Icon name="upload"/>Import GGUF</button><small>Choose a .gguf file from your device</small></div>
 <section><h3>Models to download</h3><div className="model-list">{CATALOG.map(x=>{const active=downloading===x.file;const pct=active&&downloadProgress?.name===x.file?downloadProgress.percent:-1;return <div className="model-card" key={x.file}><div className="model-info"><b>{x.name}</b><small>{x.size}</small>{active&&<div className="download-progress"><span style={{width:pct>=0?pct+'%':'35%'}}/></div>}{active&&<div className="download-meta">{pct>=0?pct+'%':'Downloading…'}{downloadProgress?.total? ' · '+fmtBytes(downloadProgress.total):''}</div>}</div><button className="model-btn" disabled={!!downloading} onClick={()=>onDownload(x)}>{active?'Downloading...':'Download'}</button></div>})}</div></section>
 <div className="section-line"/><section><h3>Downloaded models</h3><div className="model-list">{downloaded.length?downloaded.map(x=>{const size=CATALOG.find(c=>c.file===x.name)?.size||fmtBytes(x.bytes);const isLoading=loadingModel===x.name;return <div className="model-card" key={x.path}><div className="model-info"><b>{x.name}</b><small>{size}</small></div>{x.name===model?<button className="model-btn active-model" onClick={onUnload} disabled={!!loadingModel}>Unload</button>:<button className="model-btn" onClick={()=>onLoad(x.path,x.name)} disabled={!!loadingModel}>{isLoading?'Loading...':'Load'}</button>}</div>}):<div className="no-models">No downloaded models yet.</div>}</div></section></div>
}
...
function ChatSettings({cfg,onClose,onApply}){
 ...
 <div className="settings-content"><section className="settings-section"><h3>Generation</h3>
  ...
  <div className="field"><label>System prompt</label><textarea rows="6" value={v.systemPrompt} onChange={e=>setV({...v,systemPrompt:e.target.value})} placeholder="Leave empty to use the VYRA default system prompt"/><div className="prompt-actions"><small>Empty uses the built-in VYRA assistant prompt.</small><button type="button" onClick={()=>{if(window.confirm('Redefinir o system prompt para o prompt original do VYRA?'))setV({...v,systemPrompt:VYRA_SYSTEM_PROMPT})}}>Redefinir prompt do VYRA</button></div></div>
