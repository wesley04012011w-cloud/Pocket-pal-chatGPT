...
function Markdown({text='',flash=''}){let html=renderMarkdown(text);if(flash){const marker='<span class="generation-flash" aria-hidden="true">'+escapeHtml(flash)+'</span>';const i=html.lastIndexOf('</p>');html=i>=0?html.slice(0,i)+marker+html.slice(i):html+marker}return <div className="markdown" dangerouslySetInnerHTML={{__html:html}}/>}
...
const MessageRow=memo(function MessageRow({message,streaming=false,flash='',flashKey=0}){const content=String(message.content||'');const base=streaming&&flash&&content.endsWith(flash)?content.slice(0,-flash.length):content;return <article className={'message-row '+(message.role==='user'?'user':'assistant')}><div className="message-role">{message.role==='user'?'You':'Assistant'}</div><div className="message-content"><Markdown text={base} flash={streaming?flash:''}/></div></article>});
...
function App(){
 ...
 useEffect(()=>{
   const tokenListener=Llama.addListener('token',ev=>{
     ...
     const prev=streamRef.current.answer||'';const delta=p.answer.startsWith(prev)?p.answer.slice(prev.length):p.answer;const next={answer:p.answer,thinking:p.thinking,thinkingLive:p.active,flash:delta,flashKey:g.count,stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};
     streamRef.current=next;setStream(next);scheduleCommit();
   });
   const doneListener=Llama.addListener('generationDone',()=>{const g=genRef.current,p=parseThinking(g.raw);const elapsed=Math.max(.001,(performance.now()-g.start)/1000);const next={answer:p.answer,thinking:p.thinking,thinkingLive:false,flash:'',flashKey:g.count,stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};streamRef.current=next;setStream(next);commitStream();setGenerating(false);genLiveRef.current=false});
