...
function renderMarkdown(src='',flash=''){
 ...
 let html=out.join('');
 if(flash&&String(src).endsWith(String(flash)))html=markLastText(html);
 return html;
}
function markLastText(html){
 let core=html,suffix='';
 const closeRe=/<\/(?:p|li|td|th|strong|em|code|blockquote|h[1-6])>\s*$/i;
 ...
 if(!text)return html;
 const unitMatch=text.match(/(&(?:#\d+|#x[0-9a-f]+|[a-z]+);|.)$/i);
 if(!unitMatch)return html;
 const unit=unitMatch[1],idx=text.length-unit.length;
 return core.slice(0,gt+1)+text.slice(0,idx)+'<span class="generation-flash">'+unit+'</span>'+text.slice(idx)+suffix;
}
function Markdown({text='',flash=''}){return <div className="markdown" dangerouslySetInnerHTML={{__html:renderMarkdown(text,flash)}}/>}
...
function App(){
 ...
 const genRef=useRef({count:0,start:0,raw:'',targetAnswer:'',visibleAnswer:'',timer:null});
 ...
 useEffect(()=>{
   const tick=()=>{
     ...
     if(g.visibleAnswer.length<g.targetAnswer.length){
       ...
       const next={answer:g.visibleAnswer,thinking:streamRef.current.thinking,thinkingLive:streamRef.current.thinkingLive,flash:nextChar,stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};
   ...
   const tokenListener=Llama.addListener('token',ev=>{
     ...
     const elapsed=Math.max(.001,(performance.now()-g.start)/1000);
     const next={answer:g.visibleAnswer,thinking:p.thinking,thinkingLive:p.active,flash:'',stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};
   ...
   const doneListener=Llama.addListener('generationDone',()=>{
     ...
     const elapsed=Math.max(.001,(performance.now()-g.start)/1000);
     const next={answer:g.targetAnswer,thinking:p.thinking,thinkingLive:false,flash:'',stats:g.count+' tokens · '+(g.count/elapsed).toFixed(1)+' tok/s'};
 ...
 return <div className="app">
  ...
  {screen==='models'?<ModelsPage downloaded={downloaded} model={model} downloading={downloading} onBack={()=>setScreen('chat')} onImport={pickAndLoad} onDownload={download} onLoad={loadPath} onUnload={unload}/>:<main className="shell">
   ...
   <section ref={messagesRef} className="messages">{current.messages.length?<>{current.messages.map((m,i)=>{const last=i===current.messages.length-1;const shown=last&&generating&&m.role==='assistant'?{...m,content:stream.answer}:m;return <MessageRow key={i} message={shown} streaming={last&&generating&&m.role==='assistant'} flash={last&&generating&&m.role==='assistant'?stream.flash:''}/>})}</>:<div className="empty"><div className="empty-mark"><Icon name="spark"/></div><h1>Local AI, on your phone.</h1><p>Load a GGUF model and chat privately. Thinking, streaming and conversation history stay on-device.</p><button className="load" onClick={()=>setScreen('models')}>Select GGUF model</button></div>}</section>
