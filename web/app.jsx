...
function App(){
 ...
 return <div className="app">
  ...
  {screen==='models'?<ModelsPage downloaded={downloaded} model={model} downloading={downloading} onBack={()=>setScreen('chat')} onImport={pickAndLoad} onDownload={download} onLoad={loadPath} onUnload={unload}/>:<main className="shell">
   ...
   <section ref={messagesRef} className="messages">{current.messages.length?<>{current.messages.map((m,i)=>{const last=i===current.messages.length-1;const shown=last&&generating&&m.role==='assistant'?{...m,content:stream.answer}:m;return <MessageRow key={i} message={shown} streaming={last&&generating&&m.role==='assistant'}/>})}</>:<div className="empty"><div className="empty-mark"><Icon name="spark"/></div><h1>Local AI, on your phone.</h1><p>Load a GGUF model and chat privately. Thinking, streaming and conversation history stay on-device.</p><button className="load" onClick={()=>setScreen('models')}>Select GGUF model</button></div>}</section>
