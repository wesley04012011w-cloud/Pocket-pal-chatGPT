...
function App(){
 ...
 return <div className="app">
  ...
  {screen==='models'?<ModelsPage downloaded={downloaded} model={model} downloading={downloading} onBack={()=>setScreen('chat')} onImport={pickAndLoad} onDownload={download} onLoad={loadPath} onUnload={unload}/>:<main className="shell">
   ...
   <header className="floating-top"><button className="float-btn" onClick={()=>setDrawer(true)} aria-label="Menu"><Icon name="menu"/></button><div className="float-model">{model||'No model loaded'}</div><div className="float-actions"><button className="float-btn" onClick={()=>document.querySelector('.input')?.focus()} aria-label="Focus input"><Icon name="edit"/></button><button className="float-btn" onClick={openSettings} aria-label="Settings"><Icon name="settings"/></button></div></header>
   <section className="messages">{current.messages.length?<>{current.messages.map((m,i)=><MessageRow key={i} message={m}/>)}</>:<div className="empty"><div className="empty-mark"><Icon name="spark"/></div><h1>Local AI, on your phone.</h1><p>Load a GGUF model and chat privately. Thinking, streaming and conversation history stay on-device.</p><button className="load" onClick={pickAndLoad}>Select GGUF model</button></div>}</section>
